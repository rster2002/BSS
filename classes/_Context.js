...
module.exports = class Context {
    ...
    namespaceHash(value) {
        ...
        return this.buildContext.tools.quickHash(namespace + value);
    }
    ...
    registerDataChangeHandler(continueHash) {
        var hash = this.namespaceHash(continueHash);

        if (this.dataChangeHandlers[hash]) {
            this.buildContext.consoleOutput.warn("A duplicate data change handler was registered.");
            this.buildContext.consoleOutput.warn("  This is the result of using two instances of `on datachange` where the tailing commands are the same.");
            this.buildContext.consoleOutput.warn("  Use `on datachangehandler` with a handler id to make sure all handlers are unique.");
        }
        this.dataChangeHandlers[hash] = hash;
